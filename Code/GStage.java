    ...
    public GStage() {
        ...
        for (int i = 0; i < numOfGrounds; i++) {
            if(this.Grounds[i].hasLedge){
                Ledges[ledgecount] = new Hitbox(this.Grounds[i].x - 1, this.Grounds[i].y + this.Grounds[i].H, 25, 0, 0, null, 1);
                Ledges[ledgecount + 1] = new Hitbox(this.Grounds[i].x + this.Grounds[i].W + 1, this.Grounds[i].y + this.Grounds[i].H, 25, 0, 0, null, 2);
                ledgecount += 2;
            }
        }
