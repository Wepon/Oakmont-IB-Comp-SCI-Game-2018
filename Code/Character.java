    ...
    public Hitbox[] getHitbox(boolean facingRight, double x, double y, int action, int frame, Player p) {
        ...
        if (action == 5) {
            if (frame < 2) {
                Hitbox[] Harr = new Hitbox[1];
                if (facingRight == false) {
                    Harr[0] = new Hitbox(x + (p.character.Width / 2), y + (p.character.Height / 2), 30, 180, 30, p, true, -10);
                }
                if (facingRight) {
                    Harr[0] = new Hitbox(x + (p.character.Width / 2), y + (p.character.Height / 2), 30, 0, 30, p, true, 10);
                }
                return Harr;
            }
            if (frame < 35) {
